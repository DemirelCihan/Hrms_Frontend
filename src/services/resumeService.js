...
export default class ResumeService {
  ...
  addCoverLetterToResume(resumeId, coverLetterId) {
    return axios.post(
      `http://localhost:8080/api/resumes/addCoverLetterToResume?coverLetterId=${coverLetterId}&resumeId=${resumeId}`
    );
  }

  deleteCoverLetterFromResume(resumeId) {
    return axios.delete(
      `http://localhost:8080/api/resumes/deleteCoverLetterFromResume?resumeId=${resumeId}`
    );
  }
  ...
  getByCandidateId(candidateId) {
    return axios.get(
      `http://localhost:8080/api/resumes/getByCandidateId?candidateId=${candidateId}`
    );
  }
}
