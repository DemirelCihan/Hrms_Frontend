...
export default function EmployerDetail() {
  ...
  useEffect(() => {
    if (type == "employer") {
      ...
      updatedEmployerService
        .getAll()
        .then((result) => setUpdatedEmployers(result.data.data));
    ...
    userActivationService
      .getByUserId(id)
      .then((result) => setUserActivation(result.data.data));
  }, []);
            ...
            <Grid.Column width="10">
              <Grid.Row>
                <DateLabel
                  value={
                    "Joined in " +
                    new Date(userActivation.isActivatedDate).getFullYear()
                  }
                />
                <br />
                <br />
                ...
                <Button
                  ...
                  icon="cog"
                  as={NavLink}
                  to={`/employers/employer/${id}/update`}
                />
                <Button
                  compact
                  circular
                  ...
                  content="Post a Job"
                  as={NavLink}
                  to={`/employers/employer/${id}/jobPosting/add`}
                />
              </Grid.Row>
              <Grid.Row>
                <Header>
                  <span className="detail-header">{employer.companyName}</span>
                </Header>
                <Icon name="linkify" />
                {employer.webAddress}
                <br />
                    ...
                    updatedEmployer.employer.id == id && (
                      <span>
                        ...
                        <Button
                          ...
                          fluid
                          color="black"
                          content="Waiting for Update Confirmation"
                        />
                      </span>
                    )
                )}
