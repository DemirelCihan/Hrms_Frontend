...
export default function JobPostingList({ type, itemsPerRow, id }) {
  ...
  useEffect(() => {
    if (type === "filtered") {
      ...
      jobPostingService
        .getAllActiveOnesByEmployerIdSortedByPostingDate(id)
        .then((result) => setJobPostings(result.data.data));
    }
  ...
  const handleRemoveFromFavorites = (jobPosting) => {
    favoriteJobPostings.map(
      (favoriteJobPosting) =>
        favoriteJobPosting.jobPosting?.id === jobPosting.id &&
                ...
                <Card.Content>
                  {type === "favorites" ? (
                    <Button
                      ...
                      icon="minus"
                      floated="right"
                      onClick={() => handleRemoveFromFavorites(jobPosting)}
                    />
                  ) : (
                    <Button
                      compact
                      ...
                      icon="bookmark"
                      floated="right"
                      onClick={() => handleAddToFavorites(jobPosting)}
                    />
                  )}
